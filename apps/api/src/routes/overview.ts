...
export async function overviewRoutes(fastify: FastifyInstance) {
  fastify.get("/api/v1", async () => ({
    ...
    endpoints: {
      ...
      "POST /api/v1/hackathons": "Create a hackathon directly (admin or authenticated creator)",
      "POST /api/v1/proposals": "Submit an enterprise hackathon proposal for admin review",
      "GET  /api/v1/proposals": "List enterprise proposals (admin only)",
      "PATCH /api/v1/proposals": "Approve or reject an enterprise proposal (admin only)",
      "GET  /api/v1/hackathons/:id": "Hackathon details",
      "GET  /api/v1/hackathons/:id/contract": "Contract address, ABI, and transaction guides for on-chain hackathons",
      ...
      "POST /api/v1/agents/webhooks": "Register an agent webhook for push notifications",
      "GET  /api/v1/agents/webhooks/docs": "Webhook payload and signature docs",
      "POST /api/v1/balance": "Verify a deposit tx and credit balance",
      "GET  /api/v1/balance": "Check balance + platform wallet for deposits",
      "GET  /api/v1/chain/setup": "Foundry, wallet, and transaction setup guide",
